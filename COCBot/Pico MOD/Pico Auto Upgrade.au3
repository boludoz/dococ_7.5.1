...
#CS
	Loop :
	- Check the builders amount
	- Chech if Builder head is available
	- Open upgrade menu
	- Search the "000"
	- Check upgrade button
	- Get upgrade name
	- Check if upgrade is on ignore list
	- Open upgrade window
	- Get resource type
	- Get cost
	- Evaluate if sufficent Resources
	- Launch upgrade (Warning: diferent places for button of heroes)
	- Write to the log the result

	OCR Packages :
	- Builder name + Level -> coc-build
	- Cost -> coc-bonus
	- Duration -> coc-uptime (For Bld) - coc-uptime2 (For Heroes)
#CE
...
Func AutoUpgrade()
	...
	While 1
		...
		If QuickMIS("BC1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Zero", 180, 80 + $g_iNextLineOffset, 480, 350) Then
		...
		If QuickMIS("NX", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Obstacles", 180, 80 + $g_iCurrentLineOffset - 15, 480, 80 + $g_iCurrentLineOffset + 15) <> "none" Then
		...
		If Not QuickMIS("BC1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\UpgradeButton", 120, 630, 740, 670) Then
		...
		Switch $g_aUpgradeNameLevel[1]
			Case "Barbarian King", "Archer Queen", "Grand Warden"
				$g_aUpgradeResourceCostDuration[0] = QuickMIS("N1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Resources", 690, 540, 730, 580) ; get resource
			...
			Case Else
				$g_aUpgradeResourceCostDuration[0] = QuickMIS("N1", @ScriptDir & "\imgxml\Resources\Pico Auto Upgrade\Resources", 460, 510, 500, 550) ; get resource
...
Func chkResourcesToIgnore()
	...
	Switch $iIgnoredResources
		Case 2
			MsgBox(0 + 16, "Warning about your settings...", _
					"Warning ! You selected 2 resources to ignore... That can be a problem," & @CRLF & _
					"and Auto Upgrade can be ineffective, by not launching any upgrade..." & @CRLF & _
					"I recommend you to select only one resource, not more...")
		Case 3
			MsgBox(0 + 16, "Invalid settings...", _
					"Warning ! You selected 3 resources to ignore... And you can't..." & @CRLF & _
					"With your settings, Auto Upgrade will be completely ineffective" & @CRLF & _
					"and will not launch any upgrade... You must deselect one or more" & @CRLF & _
					"ignored resource.")
	EndSwitch
