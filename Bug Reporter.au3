...
#AutoIt3Wrapper_Icon=COCBot\DebugTools\Reporter_Icon.ico
...
Global $tempFolder = @ScriptDir & "\COCBot\DebugTools\Debug_Content\"
Global $userMessage = $tempFolder & "Report_Message.txt"
Global $p7zip = @ScriptDir & "\COCBot\DebugTools\7za.exe"
Global $pcURL = @ScriptDir & "\lib\curl\curl.exe"
Global $pcURL_log = $tempFolder & "cURL_report.txt"
Global $pData = @ScriptDir & "\Debug_Data_To_Upload.7z"

Local Const $sFont = "Arial Rounded MT Bold"
...
Func Screen3()
	...
	If FileExists($pData) Then FileDelete($pData)
...
Func Screen4()
	...
	GUICtrlSetFont(-1, 10, 400, 0, $sFont)
	GUICtrlCreateLabel("There, create a thread, and paste on it given download link.", 30, 185, 382, 19)
	GUICtrlSetFont(-1, 10, 400, 0, $sFont)
	...
	While 1
		...
		Switch $NMSG
			...
			Case $btnUpload
				If FileExists($pData) Then
					...
					If $fileSize > 1048576 Then
						GUICtrlSetData($URL, "Upload pending, please wait... File size: " & Round($fileSize / 1048576, 2) & "MO")
					Else
						GUICtrlSetData($URL, "Upload pending, please wait... File size: " & Round($fileSize / 1024, 2) & "KO")
					EndIf

					ShellExecuteWait($pcURL, "https://dropfile.to/getuploadserver -o " & '"' & $pcURL_log & '"', "", "open", @SW_HIDE)
					$uploadURL = FileReadLine($pcURL_log)
					ShellExecuteWait($pcURL, "-F " & '"' & "file=@" & $pData & '"' & " " & $uploadURL & "/upload -o " & '"' & $pcURL_log & '"', "", "open")
					...
					If StringInStr($res, ":0", $STR_CASESENSE) Then
						...
						FileDelete($pData)
					Else
						GUICtrlSetData($URL, 'Error: Upload "Debug_Data_To_Upload.7z" yourself')
					EndIf
			...
			Case $btnFinish
				GUIDelete($Step4)
				ShellExecute("https://mybot.run/forums/index.php?/topic/30968-dococ-is-back-o-omg-no-fake")
				Exit
		EndSwitch
	WEnd
